...
document.addEventListener('DOMContentLoaded', function () {
    // Load images into the photostrip container
...
document.getElementById('emailForm').addEventListener('submit', function(event) {
        ...
        html2canvas(document.getElementById('photostrip')).then(canvas => {
            ...
            fetch('http://localhost:5000/send-photo', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ email, imageData, subject, message })
            })
...
// Combined functions for sending email and downloading image
function sendEmail() {
    const email = document.getElementById('email').value;
    if (!email) {
...
function testCanvasCapture() {
    html2canvas(document.getElementById('photostrip')).then(canvas => {
        document.body.appendChild(canvas); // This will add the captured canvas to your webpage for visual confirmation
    });
}
</script>
